...
class BelongsToMany extends Association
{
     ...
     * @var string
     */
    protected $_junctionEndpointName;
     ...
     *
     * @var string|\Muffin\Webservice\Model\Endpoint
     */
    protected $_through;
     ...
     *
     * @param string|\Muffin\Webservice\Model\Endpoint|null $endpoint Name or instance for the join table
     * @return \Muffin\Webservice\Model\Endpoint
     */
    public function junction($endpoint = null)
    {
        if ($endpoint === null) {
            if (!empty($this->_junctionEndpoint)) {
                return $this->_junctionEndpoint;
            }

            if (!empty($this->_through)) {
                $endpoint = $this->_through;
            } else {
                $endpointName = $this->_junctionTableName();
                $endpointAlias = Inflector::camelize($endpointName);

                $config = [];
                if (!EndpointRegistry::exists($endpointAlias)) {
                    $config = ['endpoint' => $endpointName];
                }
                $endpoint = EndpointRegistry::get($endpointAlias, $config);
            }
        }
        ...
        $this->_generateSourceAssociations($endpoint, $source);
        $this->_generateTargetAssociations($endpoint, $source, $target);
        $this->_generateJunctionAssociations($endpoint, $source, $target);
        return $this->_junctionEndpoint = $endpoint;
    }
     ...
     *
     * @param \Muffin\Webservice\Model\Endpoint $junction The junction table.
     * @param \Muffin\Webservice\Model\Endpoint $source The source table.
     * @param \Muffin\Webservice\Model\Endpoint $target The target table.
     * @return void
     */
    ...
    public function attachTo(QueryInterface $query, array $options = [])
    {
        ...
        $assoc = $this->_targetRepository->association($junction->alias());
     ...
     *
     * @param \Muffin\Webservice\Model\Endpoint $side The potential Table with ownership
     * @return bool
     */
    ...
    protected function _saveLinks(EntityInterface $sourceEntity, $targetEntities, $options)
    {
        $target = $this->target();
        $junction = $this->junction();
        $entityClass = $junction->resourceClass();
        ...
        $this->junction()->connection()->transactional(
            function () use ($sourceEntity, $targetEntities, $options) {
                ...
                foreach ($links as $entity) {
                    $this->_junctionEndpoint->delete($entity, $options);
                }
            }
        );
     ...
     * @see \Muffin\Webservice\Model\Endpoint::find()
    ...
    public function replaceLinks(EntityInterface $sourceEntity, array $targetEntities, array $options = [])
    {
        ...
        return $this->junction()->connection()->transactional(
            function () use ($sourceEntity, $targetEntities, $primaryValue, $options) {
                $foreignKey = array_map([$this->_junctionEndpoint, 'aliasField'], (array)$this->foreignKey());
                $hasMany = $this->source()->association($this->_junctionEndpoint->alias());
                ...
                if ($associationConditions) {
                    ...
                    $existing->where($associationConditions);
                }
    ...
    protected function _collectJointEntities($sourceEntity, $targetEntities)
    {
        ...
        foreach ($missing as $key) {
            $conditionSet[] = array_combine($foreignKey, $sourceKey) + array_combine($assocForeignKey, $key);
        }

        $query = $hasMany->find('all')->where($conditionSet);
    ...
    protected function _buildQuery($options)
    {
        ...
        $conditions = [];
        foreach ($keys as $id) {
            $conditions[][$this->_linkField($options)] = $id;
        }

        $property = $this->target()->association($this->junction()->alias())->property();
        $query = $this->_appendJunctionJoin($query, $conditions);
        $query->formatResults(function ($results) use ($property) {
            return $results
                ->filter(function ($row) use ($property) {
                    return isset($row[$property][0]);
                })
                ->map(function (Resource $row) use ($property) {
                    $row[$property] = $row[$property][0];

                    $row->dirty($property, false);
    ...
    {
        if ($name === null) {
            if (empty($this->_junctionEndpointName)) {
                $endpointNames = array_map('\Cake\Utility\Inflector::underscore', [
                    $this->source()->endpoint(),
                    $this->target()->endpoint()
                ]);
                sort($endpointNames);
                $this->_junctionEndpointName = implode('_', $endpointNames);
            }
            return $this->_junctionEndpointName;
        }
        return $this->_junctionEndpointName = $name;
    }
