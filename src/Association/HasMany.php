...
class HasMany extends Association
{
     ...
     *
     * @param \Cake\Datasource\RepositoryInterface $side The potential Endpoint with ownership
     * @return bool
     */
    ...
    public function saveAssociated(EntityInterface $entity, array $options = [])
    {
        ...
        $options['_sourceRepository'] = $this->source();
     ...
     * Changes are persisted in the webservice and also in the source entity.
     *
     * ### Example:
     *
     * ```
     ...
     * in the webservice and that each of them contain a primary key value.
     *
     * ### Options
     *
     ...
     * ```
     *
     * `$article->get('articles')` will contain only `[$article4]` after deleting in the webservice
     *
     * @param \Cake\Datasource\EntityInterface $sourceEntity an entity persisted in the source endpoint for
     * this association
     * @param array $targetEntities list of entities persisted in the target endpoint for
     * this association
     ...
     * be created for the passed target entities that are not already in the webservice
     ...
     * only the link for 'article 1' will be kept in webservice, the links for 'article 2' and 'article 3' will be
     ...
     * check `Endpoint::save()` for information on the accepted options.
     *
     * ### Example:
     *
     * ```
     ...
     * @param \Cake\Datasource\EntityInterface $sourceEntity an entity persisted in the source endpoint for
     * this association
     * @param array $targetEntities list of entities from the target endpoint to be linked
     ...
     * @param \Cake\Datasource\RepositoryInterface $repository The associated repository
     * @param array $remainingEntities Entities that should not be deleted
     * @param array $options list of options accepted by `Endpoint::delete()`
     * @return bool success
     */
    protected function _unlinkAssociated(array $properties, EntityInterface $entity, RepositoryInterface $repository, array $remainingEntities = [], array $options = [])
    {
        $primaryKey = (array)$repository->primaryKey();
        ...
        return $this->_unlink(array_keys($properties), $repository, $conditions, $options);
    }
     ...
     * @param \Cake\Datasource\RepositoryInterface $target The associated endpoint
     * @param array $conditions The conditions that specifies what are the objects to be unlinked
     * @param array $options list of options accepted by `Endpoint::delete()`
     * @return bool success
     */
    protected function _unlink(array $foreignKey, RepositoryInterface $target, array $conditions = [], array $options = [])
    {
     ...
     *
     * @param \Cake\Datasource\RepositoryInterface $repository the repository containing the foreign key
     * @param array $properties the list of fields that compose the foreign key
     * @return bool
     */
    protected function _foreignKeyAcceptsNull(RepositoryInterface $repository, array $properties)
    {
        return !in_array(
            false,
            array_map(
                function ($prop) use ($repository) {
                    return $repository->schema()->isNullable($prop);
                },
                $properties
            )
        );
